...
main (int argc, char** argv)
{
  ...
  reader.read<pcl::PointXYZ> ("motoman.pcd", *cloud_in);
  ...
  // double theta1 = M_PI / 3;   //追加したやつ(変換行列)
  // transformation_matrix1 (0, 0) = cos (theta1);
  // transformation_matrix1 (0, 1) = -sin (theta1);
  // transformation_matrix1 (1, 0) = sin (theta1);
  // transformation_matrix1 (1, 1) = cos (theta1);

  // // A translation on Z axis (0.4 meters)
  // transformation_matrix1 (0, 3) = 0.2;
  // transformation_matrix1 (1, 3) = -0.4;
  // transformation_matrix1 (2, 3) = -0.2;

  // pcl::transformPointCloud (*cloud_o, *cloud_icp, transformation_matrix1);
  ...
  // // A rotation matrix (see https://en.wikipedia.org/wiki/Rotation_matrix)　　//元あったやつ
  ...
  // // A translation on X,Y,Z axis
  // transformation_matrix (0, 3) = 0.2;
  // transformation_matrix (1, 3) = -0.4;
  // transformation_matrix (2, 3) = -0.2;

  double phi = -0.0362 ; //ロール
  double theta = 0.3093 -0.8 ; //ピッチ
  double psi = 2.0462 + 7*M_PI/6 +0.11 ; //ヨー
  ...
  *cloud_tr = *cloud_icp;  // We backup cloud_icp into cloud_tr for later use
  ...
  if (icp.hasConverged ())
  {
    ...
    print4x4Matrix (transformation_matrix);    //関数の名前．
  }
  else
  {
